 ...
 * @method     ChildSpyCommentQuery orderByFkUser($order = Criteria::ASC) Order by the fk_user column
 ...
 * @method     ChildSpyCommentQuery groupByFkUser() Group by the fk_user column
 ...
 *
 * @method     ChildSpyCommentQuery leftJoinUser($relationAlias = null) Adds a LEFT JOIN clause to the query using the User relation
 * @method     ChildSpyCommentQuery rightJoinUser($relationAlias = null) Adds a RIGHT JOIN clause to the query using the User relation
 * @method     ChildSpyCommentQuery innerJoinUser($relationAlias = null) Adds a INNER JOIN clause to the query using the User relation
 *
 * @method     ChildSpyCommentQuery joinWithUser($joinType = Criteria::INNER_JOIN) Adds a join clause and with to the query using the User relation
 *
 * @method     ChildSpyCommentQuery leftJoinWithUser() Adds a LEFT JOIN clause and with to the query using the User relation
 * @method     ChildSpyCommentQuery rightJoinWithUser() Adds a RIGHT JOIN clause and with to the query using the User relation
 * @method     ChildSpyCommentQuery innerJoinWithUser() Adds a INNER JOIN clause and with to the query using the User relation
 *
 ...
 *
 * @method     \Orm\Zed\User\Persistence\SpyUserQuery|\Orm\Zed\Customer\Persistence\SpyCustomerQuery|\Orm\Zed\Comment\Persistence\SpyCommentThreadQuery|\Orm\Zed\Comment\Persistence\SpyCommentToCommentTagQuery endUse() Finalizes a secondary criteria and merges it with its primary Criteria
 *
 ...
 * @method     ChildSpyComment|null findOneByFkUser(int $fk_user) Return the first ChildSpyComment filtered by the fk_user column
 ...
 * @method     ChildSpyComment requireOneByFkUser(int $fk_user) Return the first ChildSpyComment filtered by the fk_user column and throws \Propel\Runtime\Exception\EntityNotFoundException when not found
 ...
 * @method     ChildSpyComment[]|Collection findByFkUser(int|array<int> $fk_user) Return ChildSpyComment objects filtered by the fk_user column
 * @psalm-method Collection&\Traversable<ChildSpyComment> findByFkUser(int|array<int> $fk_user) Return ChildSpyComment objects filtered by the fk_user column
...
abstract class SpyCommentQuery extends ModelCriteria
{
    ...
    {
        $sql = 'SELECT `id_comment`, `fk_comment_thread`, `fk_customer`, `fk_user`, `is_deleted`, `is_updated`, `key`, `message`, `uuid`, `created_at`, `updated_at` FROM `spy_comment` WHERE `id_comment` = :p0';
        try {
    ...
    public function filterByFkUser($fkUser = null, $comparison = Criteria::EQUAL)
    {
        ...
        if (is_array($fkUser)) {
            ...
            if (isset($fkUser['min'])) {
                ...
                $this->addUsingAlias(SpyCommentTableMap::COL_FK_USER, $fkUser['min'], Criteria::GREATER_EQUAL);
                $useMinMax = true;
            }
            if (isset($fkUser['max'])) {
                if ($comparison != SprykerCriteria::BETWEEN && $comparison != Criteria::LESS_EQUAL && $comparison != Criteria::LESS_THAN) {
                ...
                $this->addUsingAlias(SpyCommentTableMap::COL_FK_USER, $fkUser['max'], Criteria::LESS_EQUAL);
                $useMinMax = true;
            }
            if ($useMinMax) {
                return $this;
        ...
        $query = $this->addUsingAlias(SpyCommentTableMap::COL_FK_USER, $fkUser, $comparison);
    ...
    public function filterByUser($spyUser, ?string $comparison = null)
    {
        if ($spyUser instanceof \Orm\Zed\User\Persistence\SpyUser) {
            return $this
                ->addUsingAlias(SpyCommentTableMap::COL_FK_USER, $spyUser->getIdUser(), $comparison);
            ...
            $this
                ->addUsingAlias(SpyCommentTableMap::COL_FK_USER, $spyUser->toKeyValue('PrimaryKey', 'IdUser'), $comparison);
