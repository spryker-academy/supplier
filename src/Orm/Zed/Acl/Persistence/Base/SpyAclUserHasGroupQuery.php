...
abstract class SpyAclUserHasGroupQuery extends ModelCriteria
{
     ...
     * </code>
     *
     * @param array[$fk_acl_group, $fk_user] $key Primary key to use for the query
    ...
    {
        $sql = 'SELECT fk_acl_group, fk_user FROM spy_acl_user_has_group WHERE fk_acl_group = :p0 AND fk_user = :p1';
        try {
    ...
    {
        $this->addUsingAlias(SpyAclUserHasGroupTableMap::COL_FK_ACL_GROUP, $key[0], Criteria::EQUAL);
        $this->addUsingAlias(SpyAclUserHasGroupTableMap::COL_FK_USER, $key[1], Criteria::EQUAL);
    ...
    public function filterByPrimaryKeys($keys)
    {
        ...
        foreach ($keys as $key) {
            $cton0 = $this->getNewCriterion(SpyAclUserHasGroupTableMap::COL_FK_ACL_GROUP, $key[0], Criteria::EQUAL);
            $cton1 = $this->getNewCriterion(SpyAclUserHasGroupTableMap::COL_FK_USER, $key[1], Criteria::EQUAL);
            $cton0->addAnd($cton1);
            $this->addOr($cton0);
        }
    ...
    public function prune($spyAclUserHasGroup = null)
    {
        if ($spyAclUserHasGroup) {
            $this->addCond('pruneCond0', $this->getAliasedColName(SpyAclUserHasGroupTableMap::COL_FK_ACL_GROUP), $spyAclUserHasGroup->getFkAclGroup(), Criteria::NOT_EQUAL);
            $this->addCond('pruneCond1', $this->getAliasedColName(SpyAclUserHasGroupTableMap::COL_FK_USER), $spyAclUserHasGroup->getFkUser(), Criteria::NOT_EQUAL);
