...
/**
 * Base class that represents a row from the 'spy_product_class' table.
 *
 *
 *
 * @package    propel.generator.src.Orm.Zed.SelfServicePortal.Persistence.Base
 */
abstract class SpyProductClass implements ActiveRecordInterface
{
    /**
     * TableMap class name
     *
     * @var string
     */
    public const TABLE_MAP = '\\Orm\\Zed\\SelfServicePortal\\Persistence\\Map\\SpyProductClassTableMap';
    ...
    /**
     * The value for the id_product_class field.
     *
     * @var        int
     */
    protected $id_product_class;
    ...
    /**
     * @var        ObjectCollection|ChildSpyProductToProductClass[] Collection to store aggregation of ChildSpyProductToProductClass objects.
     * @phpstan-var ObjectCollection&\Traversable<ChildSpyProductToProductClass> Collection to store aggregation of ChildSpyProductToProductClass objects.
     */
    ...
    /**
     * An array of objects scheduled for deletion.
     * @var ObjectCollection|ChildSpyProductToProductClass[]
     * @phpstan-var ObjectCollection&\Traversable<ChildSpyProductToProductClass>
     */
    protected $productToProductClassesScheduledForDeletion = null;

    /**
     * Initializes internal state of Orm\Zed\SelfServicePortal\Persistence\Base\SpyProductClass object.
     */
    ...
    /**
     * Compares this with another <code>SpyProductClass</code> instance.  If
     * <code>obj</code> is an instance of <code>SpyProductClass</code>, delegates to
     * <code>equals(SpyProductClass)</code>.  Otherwise, returns <code>false</code>.
     *
    ...
    /**
     * Get the [id_product_class] column value.
     *
     * @return int
     */
    public function getIdProductClass()
    {
        return $this->id_product_class;
    }
    ...
    /**
     * Set the value of [id_product_class] column.
     *
     * @param int $v New value
     * @return $this The current object (for fluent API support)
     */
    public function setIdProductClass($v)
    {
        if ($v !== null) {
            $v = (int) $v;
        }
        ...
        if (($this->isNew() && $hasDefaultValue) || $this->id_product_class !== $v) {
            $this->id_product_class = $v;
            $this->modifiedColumns[SpyProductClassTableMap::COL_ID_PRODUCT_CLASS] = true;
        }
    ...
    public function setKey($v)
    {
        ...
        if (($this->isNew() && $hasDefaultValue) || $this->key !== $v) {
            $this->key = $v;
            $this->modifiedColumns[SpyProductClassTableMap::COL_KEY] = true;
        }
    ...
    public function setName($v)
    {
        ...
        if (($this->isNew() && $hasDefaultValue) || $this->name !== $v) {
            $this->name = $v;
            $this->modifiedColumns[SpyProductClassTableMap::COL_NAME] = true;
        }
    ...
    public function hydrate(array $row, int $startcol = 0, bool $rehydrate = false, string $indexType = TableMap::TYPE_NUM): int
    {
        try {
            ...
            $col = $row[TableMap::TYPE_NUM == $indexType ? 0 + $startcol : SpyProductClassTableMap::translateFieldName('IdProductClass', TableMap::TYPE_PHPNAME, $indexType)];
            $this->id_product_class = (null !== $col) ? (int) $col : null;

            $col = $row[TableMap::TYPE_NUM == $indexType ? 1 + $startcol : SpyProductClassTableMap::translateFieldName('Key', TableMap::TYPE_PHPNAME, $indexType)];
            $this->key = (null !== $col) ? (string) $col : null;

            $col = $row[TableMap::TYPE_NUM == $indexType ? 2 + $startcol : SpyProductClassTableMap::translateFieldName('Name', TableMap::TYPE_PHPNAME, $indexType)];
            ...
            return $startcol + 3; // 3 = SpyProductClassTableMap::NUM_HYDRATE_COLUMNS.

        } catch (Exception $e) {
            throw new PropelException(sprintf('Error populating %s object', '\\Orm\\Zed\\SelfServicePortal\\Persistence\\SpyProductClass'), 0, $e);
        }
    }
    ...
    public function reload(bool $deep = false, ?ConnectionInterface $con = null): void
    {
        ...
        if ($con === null) {
            $con = Propel::getServiceContainer()->getReadConnection(SpyProductClassTableMap::DATABASE_NAME);
        }

        // We don't need to alter the object instance pool; we're just modifying this instance
        // already in the pool.

        $dataFetcher = ChildSpyProductClassQuery::create(null, $this->buildPkeyCriteria())->setFormatter(ModelCriteria::FORMAT_STATEMENT)->find($con);
     ...
     * @return void
     * @throws \Propel\Runtime\Exception\PropelException
     * @see SpyProductClass::setDeleted()
     * @see SpyProductClass::isDeleted()
     */
    public function delete(?ConnectionInterface $con = null): void
    {
        ...
        $con->transaction(function () use ($con) {
            $deleteQuery = ChildSpyProductClassQuery::create()
    ...
    public function save(?ConnectionInterface $con = null): int
    {
        ...
        return $con->transaction(function () use ($con) {
            ...
            if ($ret) {
                ...
                $this->postSave($con);
                SpyProductClassTableMap::addInstanceToPool($this);
            } else {
                $affectedRows = 0;
            }
            ...
            if ($this->productsScheduledForDeletion !== null) {
                if (!$this->productsScheduledForDeletion->isEmpty()) {
                    ...
                    foreach ($this->productsScheduledForDeletion as $entry) {
                        ...
                        $entryPk[0] = $this->getIdProductClass();
                        $entryPk[1] = $entry->getIdProduct();
                        $pks[] = $entryPk;
                    }
    ...
    protected function doInsert(ConnectionInterface $con): void
    {
        ...
        if ($this->isColumnModified(SpyProductClassTableMap::COL_ID_PRODUCT_CLASS)) {
            $modifiedColumns[':p' . $index++]  = '`id_product_class`';
        }
        if ($this->isColumnModified(SpyProductClassTableMap::COL_KEY)) {
            $modifiedColumns[':p' . $index++]  = '`key`';
        }
        if ($this->isColumnModified(SpyProductClassTableMap::COL_NAME)) {
            $modifiedColumns[':p' . $index++]  = '`name`';
        }

        $sql = sprintf(
            'INSERT INTO `spy_product_class` (%s) VALUES (%s)',
        ...
        try {
            ...
            foreach ($modifiedColumns as $identifier => $columnName) {
                switch ($columnName) {
                    case '`id_product_class`':
                        $stmt->bindValue($identifier, $this->id_product_class, PDO::PARAM_INT);
        ...
        try {
            $pk = $con->lastInsertId('spy_product_class_pk_seq');
        } catch (Exception $e) {
            throw new PropelException('Unable to get autoincrement id.', 0, $e);
        }
        if ($pk !== null) {
            $this->setIdProductClass($pk);
        }
    ...
    {
        $pos = SpyProductClassTableMap::translateFieldName($name, $type, TableMap::TYPE_NUM);
    ...
    {
        if (isset($alreadyDumpedObjects['SpyProductClass'][$this->hashCode()])) {
            return ['*RECURSION*'];
        }
        $alreadyDumpedObjects['SpyProductClass'][$this->hashCode()] = true;
        $keys = SpyProductClassTableMap::getFieldNames($keyType);
        $result = [
            $keys[0] => $this->getIdProductClass(),
            $keys[1] => $this->getKey(),
            $keys[2] => $this->getName(),
        ];
    ...
    public function fromArray(array $arr, string $keyType = TableMap::TYPE_FIELDNAME)
    {
        ...
        if (array_key_exists($keys[0], $arr)) {
            $this->setIdProductClass($arr[$keys[0]]);
        }
    ...
    public function buildCriteria(): Criteria
    {
        ...
        if ($this->isColumnModified(SpyProductClassTableMap::COL_ID_PRODUCT_CLASS)) {
            $criteria->add(SpyProductClassTableMap::COL_ID_PRODUCT_CLASS, $this->id_product_class);
        }
        if ($this->isColumnModified(SpyProductClassTableMap::COL_KEY)) {
            $criteria->add(SpyProductClassTableMap::COL_KEY, $this->key);
        }
        if ($this->isColumnModified(SpyProductClassTableMap::COL_NAME)) {
            $criteria->add(SpyProductClassTableMap::COL_NAME, $this->name);
        }
    ...
    {
        $criteria = ChildSpyProductClassQuery::create();
        $criteria->add(SpyProductClassTableMap::COL_ID_PRODUCT_CLASS, $this->id_product_class);
    ...
    {
        $validPk = null !== $this->getIdProductClass();
    ...
    /**
     * Generic method to set the primary key (id_product_class column).
     *
    ...
    {
        return null === $this->getIdProductClass();
    }
     ...
     * objects.
     *
     * @param object $copyObj An object of \Orm\Zed\SelfServicePortal\Persistence\SpyProductClass (or compatible) type.
    ...
    public function copyInto(object $copyObj, bool $deepCopy = false, bool $makeNew = true): void
    {
        ...
        if ($makeNew) {
            $copyObj->setNew(true);
            $copyObj->setIdProductClass(NULL); // this is a auto-increment column, so set to default value
        }
    }
     ...
     *
     * @param bool $deepCopy Whether to also copy all rows that refer (by fkey) to the current row.
     * @return \Orm\Zed\SelfServicePortal\Persistence\SpyProductClass Clone of current object.
     * @throws \Propel\Runtime\Exception\PropelException
     */
    ...
    /**
     * Gets an array of ChildSpyProductToProductClass objects which contain a foreign key that references this object.
     *
     * If the $criteria is not null, it is used to always fetch the results from the database.
     * Otherwise the results are fetched from the database the first time, then cached.
     * Next time the same method is called without $criteria, the cached collection is returned.
     * If this ChildSpyProductClass is new, it will return
     * an empty collection or the current collection; the criteria is ignored on a new object.
     *
     * @param Criteria $criteria optional Criteria object to narrow the query
     * @param ConnectionInterface $con optional connection object
     * @return ObjectCollection|ChildSpyProductToProductClass[] List of ChildSpyProductToProductClass objects
     * @phpstan-return ObjectCollection&\Traversable<ChildSpyProductToProductClass> List of ChildSpyProductToProductClass objects
     * @throws \Propel\Runtime\Exception\PropelException
     */
    public function getProductToProductClasses(?Criteria $criteria = null, ?ConnectionInterface $con = null)
    {
        ...
        if (null === $this->collProductToProductClasses || null !== $criteria || $partial) {
            if ($this->isNew()) {
                ...
                $collProductToProductClasses = ChildSpyProductToProductClassQuery::create(null, $criteria)
                    ->filterByProductClass($this)
                    ->find($con);
    ...
    /**
     * Sets a collection of ChildSpyProductToProductClass objects related by a one-to-many relationship
    ...
    {
        /** @var ChildSpyProductToProductClass[] $productToProductClassesToDelete */
        ...
        foreach ($productToProductClassesToDelete as $productToProductClassRemoved) {
            $productToProductClassRemoved->setProductClass(null);
        }
    ...
    /**
     * Returns the number of related SpyProductToProductClass objects.
     *
     * @param Criteria $criteria
     * @param bool $distinct
     * @param ConnectionInterface $con
     * @return int Count of related SpyProductToProductClass objects.
     * @throws \Propel\Runtime\Exception\PropelException
     */
    public function countProductToProductClasses(?Criteria $criteria = null, bool $distinct = false, ?ConnectionInterface $con = null): int
    {
        ...
        if (null === $this->collProductToProductClasses || null !== $criteria || $partial) {
            ...
            $query = ChildSpyProductToProductClassQuery::create(null, $criteria);
            if ($distinct) {
                $query->distinct();
            }
    ...
    /**
     * Method called to associate a ChildSpyProductToProductClass object to this object
     * through the ChildSpyProductToProductClass foreign key attribute.
     *
     * @param ChildSpyProductToProductClass $l ChildSpyProductToProductClass
     * @return $this The current object (for fluent API support)
     */
    public function addProductToProductClass(ChildSpyProductToProductClass $l)
    {
        if ($this->collProductToProductClasses === null) {
            $this->initProductToProductClasses();
            $this->collProductToProductClassesPartial = true;
    ...
    /**
     * @param ChildSpyProductToProductClass $productToProductClass The ChildSpyProductToProductClass object to add.
     */
    protected function doAddProductToProductClass(ChildSpyProductToProductClass $productToProductClass): void
    {
        $this->collProductToProductClasses[]= $productToProductClass;
        $productToProductClass->setProductClass($this);
    }

    /**
     * @param ChildSpyProductToProductClass $productToProductClass The ChildSpyProductToProductClass object to remove.
     * @return $this The current object (for fluent API support)
     */
    public function removeProductToProductClass(ChildSpyProductToProductClass $productToProductClass)
    {
        if ($this->getProductToProductClasses()->contains($productToProductClass)) {
            ...
            $productToProductClass->setProductClass(null);
        }
     ...
     * Otherwise if this SpyProductClass is new, it will return
     * an empty collection; or if this SpyProductClass has previously
     * been saved, it will retrieve related ProductToProductClasses from storage.
     *
     * This method is protected by default in order to keep the public
     * api reasonable.  You can provide public methods for those you
     * actually need in SpyProductClass.
     *
     * @param Criteria $criteria optional Criteria object to narrow the query
     * @param ConnectionInterface $con optional connection object
     ...
     * @phpstan-return ObjectCollection&\Traversable<ChildSpyProductToProductClass}> List of ChildSpyProductToProductClass objects
     */
    public function getProductToProductClassesJoinProduct(?Criteria $criteria = null, ?ConnectionInterface $con = null, $joinBehavior = Criteria::LEFT_JOIN)
    {
        $query = ChildSpyProductToProductClassQuery::create(null, $criteria);
    ...
    public function initProducts()
    {
        ...
        $this->collProducts->setModel('\Orm\Zed\Product\Persistence\SpyProduct');
    }
     ...
     * @return ChildSpyProductClass The current object (for fluent API support)
     */
    public function addProduct(SpyProduct $product)
    {
     ...
     * @param SpyProduct $product
     */
    protected function doAddProduct(SpyProduct $product)
    {
        $spyProductToProductClass = new ChildSpyProductToProductClass();

        $spyProductToProductClass->setProduct($product);

        $spyProductToProductClass->setProductClass($this);

        $this->addProductToProductClass($spyProductToProductClass);

        // set the back reference to this object directly as using provided method either results
        // in endless loop or in multiple relations
        if (!$product->isProductClassesLoaded()) {
            $product->initProductClasses();
            $product->getProductClasses()->push($this);
        } elseif (!$product->getProductClasses()->contains($this)) {
            $product->getProductClasses()->push($this);
        }
     ...
     * @return ChildSpyProductClass The current object (for fluent API support)
     */
    public function removeProduct(SpyProduct $product)
    {
        if ($this->getProducts()->contains($product)) {
            $spyProductToProductClass = new ChildSpyProductToProductClass();
            $spyProductToProductClass->setProduct($product);
            if ($product->isProductClassesLoaded()) {
                ...
                $product->getProductClasses()->removeObject($this);
            }
    ...
    {
        return (string) $this->exportTo(SpyProductClassTableMap::DEFAULT_STRING_FORMAT);
    }
